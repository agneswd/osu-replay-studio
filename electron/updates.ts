...
export interface UpdateStatus { state: "disabled" | "idle" | "checking" | "downloading" | "ready" | "error"; version: string; nextVersion?: string; percent?: number }
export function startUpdates() {
  ...
  if (enabled) {
    ...
    autoUpdater.on("update-available", info => { status = { ...status, state: "downloading", nextVersion: info.version, percent: 0 }; });
