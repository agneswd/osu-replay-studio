...
The intro and outro each run for 5.4 seconds. Gameplay holds for one second after the intro, then eases from a stop to 1x. The first note is one second of gameplay after motion starts. The outro starts as soon as the replay has no frames left. Music continues at half volume through the outro and fades with the final black transition.
