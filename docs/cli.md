...
The intro and outro each run for 5.4 seconds. Gameplay holds for one second from when the background starts clearing, then accelerates from zero to 1x over 1 second. Motion overlaps the last 0.3 seconds of the magic-lamp close. Music fades to half volume during acceleration, then returns to full volume over 0.25 seconds. The frozen source frame is one second before the first note. Acceleration starts from that frame and reaches 1x before the note. Recorded cursor data and object animations finish, then gameplay fades into the dimmed background for one second before the outro. Music continues at half volume through the outro and fades with the final black transition.
