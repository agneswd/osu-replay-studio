...
(async () => {
  ...
  try {
    ...
    const click = async label => {
      await js(`(() => { const button = [...(document.querySelector('[role="dialog"]') ?? document).querySelectorAll('button,[role="tab"]')].find(e => e.getAttribute('aria-label') === ${JSON.stringify(label)} || e.textContent.trim() === ${JSON.stringify(label)}); if (!button) throw Error('Missing button'); button.click(); })()`);
      if (label === "Done") await wait(`!document.querySelector('[role="dialog"]')`);
    };
    ...
    if (!before) {
      ...
      await click("Player and map details");
      await wait(`!!document.getElementById('youtube-playerUrl')`);
      await fill("youtube-playerUrl", "https://osu.ppy.sh/users/42");
      await click("Player and map details");
      await fill("youtube-pp", "412");
      await click("Play status");
      await wait(`[...document.querySelectorAll('[role="option"]')].some(e => e.textContent.includes('Misses'))`);
      await js(`[...document.querySelectorAll('[role="option"]')].find(e => e.textContent.includes('Misses')).click()`);
      await wait(`!!document.getElementById('youtube-status-count')`);
      await fill("youtube-status-count", "2");
      await js(`document.getElementById('youtube-status-count').dispatchEvent(new FocusEvent('focusout', { bubbles: true }))`);
      await wait(`document.getElementById('youtube-title').value.includes('2xMiss')`);
      await capture("status-count");
      await click("Play status");
      await wait(`[...document.querySelectorAll('[role="option"]')].some(e => e.textContent.includes('Full combo'))`);
      await js(`[...document.querySelectorAll('[role="option"]')].find(e => e.textContent.includes('Full combo')).click()`);
      await fill("youtube-youtubeUrl", "https://www.youtube.com/@player");
      await fill("youtube-twitchUrl", "https://www.twitch.tv/player");
      await wait(`document.getElementById('youtube-description').value.includes('Twitch: https://www.twitch.tv/player')`);
      ...
      assert.equal(await value("youtube-youtubeUrl"), "");
      assert.equal(await value("youtube-twitchUrl"), "");
